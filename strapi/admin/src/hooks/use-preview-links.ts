...
const usePreviewLinks = (api?: string, isDraft?: boolean, id?: any) => {
  ...
  const { links, isLoading, isLoaded } = useSelector((state: any) => state[REDUCER.ID] as IReducerState);
  ...
  const loadData = useCallback(
    async () => {
      try {
        ...
        const response = await fetchClient.get('/futurebrand-strapi-helpers/preview', {
          params: {
            id,
            api,
            draft: isDraft,
